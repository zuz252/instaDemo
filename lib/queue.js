...
const SEED_DEMOERS = [
  { name: "Ada Lovelace", project: "Analytical Engine, but it vibes" },
  { name: "Linus Torvalds", project: "Yet another operating system" },
  { name: "Grace Hopper", project: "COBOL but make it fun" },
  { name: "Alan Turing", project: "Universal Machine, live edition" },
  { name: "Margaret Hamilton", project: "Apollo guidance, for cats" },
];
...
function seedDemoers() {
  const added = [];
  for (const { name, project } of SEED_DEMOERS) {
    const demoer = {
      id: state.nextId++,
      name,
      project,
